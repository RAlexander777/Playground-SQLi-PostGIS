...
This article presents an experimental evaluation of spatial SQL injection vulnerabilities in PostGIS-based cadastral systems. The study: (1) documents and evaluates six attack vectors mapped to the CIA triad (Confidentiality, Integrity, and Availability); (2) implements a reproducible testbed over an experimental dataset of 1,500 urban parcels structured under the ISO 19152 (LADM) standard; and (3) quantifies the latency and throughput impact of a mitigation architecture combining strongly typed ORM compilation (GeoAlchemy2) with pre-execution validation via Shapely and Pydantic.
...
Seminal literature in computer security has established robust taxonomies for SQLi attacks. Halfond et al. (2006) categorized these threats into tautologies, union queries, blind injections, and stored procedure piggybacking. Clarke (2012) expanded upon this by examining the abstract syntax trees (AST) generated by database parsers when unsanitized parameters are concatenated. However, these classical frameworks fail to account for how spatial algebraic operators (Egenhofer, 1994) redefine logical evaluation conditions. Unlike scalar values, a geometric expression processes multiple dimensions, spatial reference system identifiers (SRID), and minimum bounding boxes, significantly expanding the syntactic escape surface. This theoretical gap in classical injection models leaves topological predicate manipulation in spatial relational engines unaddressed.

### 2.2. Geospatial Access Control Models and Spatial Inference Breaches
Access control in spatial databases was formally conceptualized by Bertino et al. (2005) through the GEO-RBAC (*Spatially Aware Role-Based Access Control*) model, demonstrating that read and write privileges must be dynamically subordinated to geographic boundaries. Complementarily, Chun and Atluri (2008) and Atluri and Chun (2004) investigated spatial inference risks in SDBMS, where unauthorized actors deduce the presence or attributes of protected spatial entities by correlating successive queries. Nevertheless, these approaches assume that the application layer faithfully projects spatial policies to persistence; in modern web architectures, unsafe concatenation directly nullifies persistence-tier access controls without detection by application-tier authorization mechanisms.

### 2.3. Cadastral Integrity and Data Modeling under ISO 19152 (LADM)
The technical modeling of cadastral information systems is governed internationally by the ISO 19152:2012 standard, known as the Land Administration Domain Model (LADM). Foundational works by Lemmen et al. (2015) and van Oosterom et al. (2006) formalized core cadastral entities, prominently `LA_SpatialUnit` (parcels and lots) and `LA_Party` (property owners), subsequently extended toward automated mass valuation models (Kara et al., 2021). While the standard prescribes the semantics and interoperability of territorial and registry data, it omits query-level architectural safeguards against SQL injection attacks, leaving fiscal revenue and title tenure integrity exposed to software implementation flaws.

### 2.4. Algorithmic Complexity Attacks and Vulnerabilities in OGC Services
The contemporary relevance of this threat was acutely illustrated in the software industry by critical vulnerability CVE-2023-25157 (CVSS score 9.8), wherein the reference software GeoServer allowed unauthenticated remote SQL injection via flawed evaluation of OGC filters and CQL expressions over PostGIS backends (National Vulnerability Database, 2023). In parallel, regarding denial of service, Crosby and Wallach (2003) established the paradigm of Algorithmic Complexity Attacks, demonstrating how engineered inputs can force worst-case execution paths ($O(N^2)$) to monopolize CPU resources. In spatial engines, as documented by Agarwal and Rajan (2016), computational geometry algorithms in PostGIS's underlying GEOS library exhibit quadratic computational overhead when evaluating complex polygons devoid of preliminary GiST spatial index filtering. Despite these precedents, the literature has neither quantified the resulting latency profiles nor formalized multi-tier defense schemes integrating API parsers with geometric engines.
...
A quantitative, experimental design was adopted to evaluate the susceptibility of geospatial endpoints to SQL injections and measure the performance impact introduced by a multi-tier mitigation scheme. The evaluation compares two conditions: unprotected dynamic SQL queries versus ORM-compiled queries with prior geometric validation.
...
* **Result:** The query transformed into an unconditional deletion (`DELETE FROM tg_lote WHERE cod_sector = '0101' OR '1'='1'`), purging all 1,500 parcels and clearing the web viewer. In contrast, the mitigated endpoint validated the sector parameter using the strict regex pattern `^\d{4}$`, rejecting the injection with HTTP 422 and preserving the cadastral repository intact.
